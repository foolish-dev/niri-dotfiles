...
|---|---|
| `dotctl install` | Wire Chaotic AUR + BlackArch (both idempotent), `cargo install` grogu, clone+venv HexStrike AI, `pacman -S` tmux/fastfetch/neovim/noctalia-shell, `yay -S` `sddm-theme-noctalia-git` (a themed SDDM fallback), `pacman -S` `greetd`/`greetd-regreet`/`cage` (then write the greetd + noctalia-themed ReGreet config under `/etc/greetd/` and enable `greetd.service` as a graphical login screen — disabling `sddm`, but never overriding a third-party display manager), and build `noctalia-unofficial-auth-agent-git` from a locally patched PKGBUILD (GCC 16 fix). |
