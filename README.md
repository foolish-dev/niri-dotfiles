...
| `dotctl deploy` | Symlink `.config/{teleia,nvim,noctalia,fastfetch,tmux}` and every `.local/bin/*` into `$HOME`, and the curated `wallpapers/*` into `~/Pictures/Wallpapers` (the dir noctalia watches; applied on all monitors). Symlink the hexstrike systemd unit, `daemon-reload`, then `enable --now` it plus the package-provided `bb-auth.service`. Back up displaced files. |
